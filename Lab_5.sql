...
INSERT INTO "Borrowed_books" (borrow_ID, book_ID, customer_ID, borrow_date, return_date)
VALUES
...
-- Books that have never borrowed

SELECT Title FROM "Books" left join "Borrowed_books" Bb on "Books".book_id = Bb.book_id WHERE bb.book_id is null;

-- Books with authors born before 1930
SELECT "Books".*  FROM "Books" left join "Authors" A on "Books".author_id = A.author_id WHERE(SELECT extract(YEAR FROM birth_date)) <= 1930;

-- Add lines to authors
INSERT INTO "Authors" (author_ID, first_name, last_name, birth_date)
VALUES
(21, 'Haruki', 'Murakami', '12/01/1949'),
(22, 'Nassim', 'Taleb', '11/09/1960'),
(23, 'Ray', 'Bradbury', '22/08/1920');

-- Authors have not published any books
SELECT "Authors".* FROM "Authors" left join "Books" on "Authors".author_id = "Books".author_id WHERE "Books".author_id is null;

-- Authors with max number of books
SELECT "Authors".first_name, count(B.book_id) as book_count from "Authors"
LEFT JOIN "Books" B on "Authors".author_id = B.author_id
GROUP BY "Authors".author_id
ORDER BY book_count DESC
LIMIT 15;

-- Customers with books containing word potter
SELECT "Customers".* FROM "Customers" LEFT JOIN "Borrowed_books" Bb on "Customers".customer_id = Bb.customer_id
JOIN "Books" B on B.book_id = Bb.book_id
WHERE B.title like '%Potter%';

-- Remove books where return date is later than april
DELETE FROM "Borrowed_books" WHERE (extract(month from return_date)) >= 4;

-- Update student with id 5
UPDATE "Customers"
SET email = 'nurzhanov.e@gmail.com'
WHERE customer_id = 5;

-- Find book which was borrowed by customer with last name Ospanov
SELECT "Borrowed_books".book_id from "Borrowed_books" join "Customers" C on C.customer_id = "Borrowed_books".customer_id WHERE C.last_name = 'Ospanov';

-- Books borrowed on mondays
SELECT * FROM "Borrowed_books" WHERE (extract(DOW FROM borrow_date) = 1)

-- Books before the 1990 with authors
SELECT "Books".*, "Authors".first_name, "Authors".last_name FROM "Books" LEFT JOIN "Authors" on "Authors".author_id = "Books".author_id;

-- Retrieve books with the total number of times each book has been borrowed
SELECT "Books".*, count(B.book_id) as book_count from "Books"
LEFT JOIN "Borrowed_books" B on B.book_id= "Books".book_id
