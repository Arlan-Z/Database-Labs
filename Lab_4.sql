...
-- count books
SELECT count(book_id) as book_count from "Books" where publication_year < 2000;

-- List books with additional columns
SELECT title, publication_year,
CASE
    WHEN publication_year < 1920 THEN 'Before 1920'
    WHEN publication_year > 1920 AND publication_year < 1980 THEN 'Between 1920 and 1980'
    ELSE 'After 1980'
END AS Info
FROM "Books";

-- Retrieve in upper case
SELECT UPPER(first_name) AS upper_name from "Customers";

-- count of whom borrowed a book
SELECT count(distinct customer_id) from "Borrowed_books";

-- list authors who have have a dot
SELECT *
FROM "Authors"
...
SELECT "left"(last_name, 3) FROM "Authors";

-- Find book with earliest year
SELECT * FROM "Books"
...
-- Combine title and year in one column
SELECT CONCAT(title, ' - ', publication_year)
FROM "Books";

-- Phone number without prefix
SELECT first_name, last_name, "right"(phone_number, 7) from "Customers";


-- Interval between time
SELECT date_part('day',borrow_date - return_date) FROM "Borrowed_books";

-- length of authors name]
SELECT length(concat(first_name, ' ',  last_name)) from "Authors";

-- Retrive publication year with more than 3 books
SELECT publication_year, count(*) from "Books" group by publication_year having count(*) >= 3;